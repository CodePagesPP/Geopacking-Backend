...
public class AyudanteController {
    ...
    @PostMapping
    public ResponseEntity<UserDTO> registerAyudante(@RequestBody AyudanteDTO AyudanteDTO) {
        return new ResponseEntity<>(ayudanteService.registerReporte(AyudanteDTO), HttpStatus.CREATED);
    }

    @GetMapping
    public ResponseEntity<List<UserDTO>> getAllAyudante() {
        return ResponseEntity.ok(ayudanteService.getAllReportes());
    }

    @GetMapping("/{id}")
    public ResponseEntity<UserDTO> getAyudanteById(@PathVariable long id) {
        return ResponseEntity.ok(ayudanteService.getReporteById(id));
    }

    @PutMapping("/{id}")
    public ResponseEntity<UserDTO> updateAyudante(@PathVariable long id, @RequestBody AyudanteDTO AyudanteDTO) {
        return ResponseEntity.ok(ayudanteService.updateReporte(id, AyudanteDTO));
    }

    @DeleteMapping("/{id}")
    public ResponseEntity<Void> deleteAyudante(@PathVariable long id) {
